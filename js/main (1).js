...
const MainSlider = (function () {
  ...
  function pagination() {
    ...
    sliderInstance.on('slideChangeTransitionStart', function () {
      const active = pagination.querySelector('.is-active');
      if (active) active.classList.remove('is-active');
      if (items[sliderInstance.realIndex]) {
        items[sliderInstance.realIndex].classList.add('is-active');
      }
    });

    items.forEach((el, i) => {
      el.addEventListener('click', () => {
        sliderInstance.slideToLoop(i); // Use slideToLoop for loop mode
      });
    });
  }
...
function counter(target, animDelay = 0) {
  const counterVal = parseFloat(target.getAttribute('data-counter'));
  const counterAdd = target.getAttribute('data-counter-add') || '';
  const isSmall = counterVal <= 50; // add precision for smaller numbers
  ...
  gsap.to(obj, {
    ...
    onUpdate: () => {
      const value = isSmall
        ? obj.count.toFixed(1)
        : Math.round(obj.count).toLocaleString();
      target.textContent = value + counterAdd;
    }
  });
}
  ...
  const observer = new IntersectionObserver((entries, obs) => {
    entries.forEach(entry => {
      if (entry.isIntersecting && !hasAnimated) {
        ...
        obs.unobserve(entry.target); // only unobserve that specific section
      }
    });
  }, {
    threshold: 0.3
...
function masonryFilterInit() {
  ...
  filterGrids.forEach(gridSection => {
    ...
    const filterButtons = gridSection.querySelectorAll('.filter-button-group button');
